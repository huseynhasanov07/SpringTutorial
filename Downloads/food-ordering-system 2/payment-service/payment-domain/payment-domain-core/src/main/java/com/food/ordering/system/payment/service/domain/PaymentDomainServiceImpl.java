...
public class PaymentDomainServiceImpl implements PaymentDomainService {
    ...
    public PaymentEvent validateAndCancelPayment(
        ...
        if (failureMessages.isEmpty()) {
            ...
            return new PaymentCancelledEvent(payment, ZonedDateTime.now(ZoneId.of(UTC)), failureMessages, paymentCancelledEventDomainEventPublisher);
        } else {
    ...
    private void validateCreditEntry(Payment payment, CreditEntry creditEntry, List<String> failureMessages) {
        if (payment.getPrice().isGreaterThan(creditEntry.getTotalCreditAmount())) {
            log.error("Customer with creditHistoryId: {} doesn't have enough credit for payment", payment.getCustomerId().getValue());
            failureMessages.add("Customer with creditHistoryId=" + payment.getCustomerId().getValue() + "doesn't have enough credit for payment!");
        }
    }
