...
public class Restaurant extends BaseEntity<RestaurantId> {
    ...
    public void validateOrder(List<String> failureMessages) {
        ...
        Money totalAmount = orderDetail.getProducts().stream().map(
                product ->
                {
                    if (!product.isAvailable()) {
                        failureMessages.add("Product with orderApprovalId: " + product.getId().getValue() + " is not available");
                    }
                    return product.getPrice().multiply(product.getQuantity());
                }).reduce(Money.ZERO, Money::add);
        if (!totalAmount.equals(orderDetail.getTotalAmount())) {
            failureMessages.add("Price total is not correct for order orderApprovalId: " + orderDetail.getId());
        }
    }
