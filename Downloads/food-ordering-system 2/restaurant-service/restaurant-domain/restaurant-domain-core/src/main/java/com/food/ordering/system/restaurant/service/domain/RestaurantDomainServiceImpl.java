...
public class RestaurantDomainServiceImpl implements RestaurantDomainService {
    ...
    public OrderApprovalEvent validateOrder(Restaurant restaurant, List<String> failureMessages, DomainEventPublisher<OrderApprovedEvent> orderApprovedEventDomainEventPublisher, DomainEventPublisher<OrderRejectedEvent> orderRejectedEventDomainEventPublisher) {
        ...
        log.info("Validating order with restaurantId: {}", restaurant.getOrderDetail().getId().getValue());
        if (failureMessages.isEmpty()) {
            log.info("Order is approved for order restaurantId: {}", restaurant.getOrderDetail().getId().getValue());
            ...
            log.info("Order is rejected for order restaurantId: {}", restaurant.getOrderDetail().getId().getValue());
